from scipy import matrix, rand, savetxt, zeros, linalg
from time import perf_counter
import matplotlib.pyplot as plt
from numpy import fill_diagonal, longdouble, matrix    #!!!
...
for i in range(Ncorridas):
    ...
    name = (f"caso_2_longdouble{i}.txt")  #!!!
    fid = open(name, "w")
    ...
    #lap
        ...
        A_inv = linalg.inv(matrix(A), overwrite_a=False)   #!!!
...
for a in range(Ncorridas):
    ...
    name = open(f"caso_2_longdouble{a}.txt", "r")      #!!!
    Xaxis = []
    Ytime = []
    Ymemory = []
    lines = []
...
#obtención de datos de cada linea de los archivos
    ...
    plt.xscale("log")
    plt.yscale("log")
    plt.title("Rendimiento Inv (caso 2 longdouble)")     #!!!
