...
for i in range(Ncorridas):
    ...
    name = (f"caso_3_longdouble{i}.txt")  #!!!
    fid = open(name, "w")
    ...
    #lap
        ...
        A_inv = linalg.inv(matrix(A), overwrite_a=True)   #!!!
...
for a in range(Ncorridas):
    ...
    name = open(f"caso_3_longdouble{a}.txt", "r")      #!!!
    Xaxis = []
    Ytime = []
    Ymemory = []
    lines = []
...
#obtención de datos de cada linea de los archivos
    ...
    plt.xscale("log")
    plt.yscale("log")
    plt.title("Rendimiento Inv (caso 3 longdouble)")     #!!!
